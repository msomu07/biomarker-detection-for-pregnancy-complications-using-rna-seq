...
second_trimester_data_df = read.table("~/Desktop/HORIZONS-Pregnancy-Data/second_trimester_data.txt", header = TRUE)
second_trimester_data = data.matrix(second_trimester_data_df)
#head(preeclampsia_firstT)
diseases_df = read.table("~/Desktop/HORIZONS-Pregnancy-Data/second_trimester_diseases.txt", header = TRUE)
diseases = diseases_df$disease
#ncol(second_trimester_data)
#length(diseases)

d <- DGEList(counts=second_trimester_data,group=factor(diseases)) 
d
...
topTags(et43)
write.table(et43, "preeclamspsia_diffe_secondT.txt")
...
topTags(et13)
write.table(et13, "chronic_hypertension_diffe_secondT.txt")
...
topTags(et23)
write.table(et23, "gestational_diabetes_diffe_secondT.txt")
